 ...
 * <!-- Root Logger -->
	...
	System.out.println() 을 사용해 로그를 확인할 수 있으나 로그를 기록하는 방법을 사용한다.
	
	log4j은 다음과 같은 레벨을 갖는다.
	TRACE > DEBUG > INFO > WARN > ERROR > FATAL
	만약에 INFO로 세팅하면 INFO, ERROR, FATAL은 기록된다.
	
	FATAL : 심각한 에러 -> 프로그램 자체가 실행되자 않을 수 있다.
	ERROR : 문제가 발생하면 소프트웨어 일부가 기능을 수행하지 못할 수 있다.
	WARNING : 소프트웨어가 동작하고 있지만, 예상치 못한 일이 발생하거나 가까운 미래에 문제가 발생할 수 있다는 경고이다.
			  예로 디스크 공간 부족 -> 프로그램은 동작되지만 앞으로 문제라 발생할 수도 있음.
